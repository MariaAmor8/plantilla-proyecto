...
public interface Operacion_prestamoRepository extends JpaRepository<Operacion_cuenta, Integer>{

    @Query(value = "SELECT * FROM operaciones_cuentas", nativeQuery = true)
    Collection<Operacion_prestamo> darOperaciones();

    @Query(value = "SELECT * FROM operaciones_cuentas WHERE id = :id", nativeQuery = true)
    Operacion_prestamo darOperacion(@Param("id") Integer id);
